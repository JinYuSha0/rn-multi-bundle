...
program
  ...
  .option('-e, --entry [path]', 'Entry file path', parseFilepath, path.join(process.cwd(),'./index.js'))
  .option('-c, --config [path]', 'Config file path', parseFilepath, path.join(__dirname, './bundleSplit.conf.js'))
  .option('-o, --out [path]', 'Output folder path', parseDirpath, '')
  .option('-a, --assetsOut [path]', 'Assets folder output path', parseDirpath, '')
  .option('-s, --sourceOut [path]', 'Source folder output filepath', parseDirpath, path.join(process.cwd(),'./multiBundle'))
  .parse();
...
if (!options.out && options.buz) {
	options.out = path.join(process.cwd(), `./multiBundle/buzBundle/${options.platform}`)
}
...
if (!options.assetsOut && options.buz) {
	options.assetsOut = path.join(process.cwd(), `./multiBundle/buzBundle/${options.platform}`)
}
